 ...
 *   Change database name <<AX2012_Plutao>> to your own DB names (Ctrl + H)
 *
 * After Running-
...
SELECT *
INTO AX2012_Plutao.dbo.SQLDICTIONARY_BAK
FROM AX2012_Plutao.dbo.SQLDICTIONARY

SELECT *
INTO AX2012_Plutao.dbo.SYSTEMSEQUENCES_BAK
FROM AX2012_Plutao.dbo.SYSTEMSEQUENCES
...
DELETE AX2012_Plutao.dbo.SQLDICTIONARY
...
WITH t AS (
    ...
    LEFT OUTER JOIN AX2012_Plutao.dbo.SQLDictionary s
...
UPDATE AX2012_Plutao.dbo.SYSTEMSEQUENCES
SET TABID = t.axid
FROM t
JOIN AX2012_Plutao.dbo.SYSTEMSEQUENCES x
...
WITH t AS (
    SELECT
        ...
        s.SQLNAME
    FROM modelelementdata md,ModelElement m
    LEFT OUTER JOIN AX2012_Plutao..SQLDictionary s
    ON upper(m.NAME) collate Latin1_General_CI_AS = s.NAME
    WHERE m.ElementType = 44 -- UtilElementType::Table
        AND m.elementhandle = md.elementhandle
        AND s.ARRAY = 0
...
UPDATE AX2012_Plutao.dbo.SQLDICTIONARY
...
FROM t
JOIN AX2012_Plutao.dbo.SQLDICTIONARY s
...
WHERE AX2012_Plutao.dbo.SQLDICTIONARY.TABLEID < 0
GO
...
FROM t join AX2012_Plutao.dbo.SQLDICTIONARY s
